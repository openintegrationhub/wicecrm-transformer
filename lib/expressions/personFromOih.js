...
module.exports.getExpression = msg => {
  ...
  let oihApplicationRecords = `${JSON.stringify(msg.body.oihApplicationRecords)}`;
  let recordUid;

  msg.body.oihApplicationRecords.reduce(record => {
    let setIds = [];
    if (record.recordUid !== undefined) {
      recordUid = (jsonata(`$filter(${oihApplicationRecords}, function($v) { $v.applicationUid = ''})`).evaluate()).recordUid;
