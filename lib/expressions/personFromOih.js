...
module.exports.getExpression = msg => {
  ...
  const lastModifications = (jsonata(`$filter(${JSON.stringify(msg.body.oihApplicationRecords)}, function($v, $i, $a) { ($v.lastModified != '') })`).evaluate());
  ...
  const expression = {
    ...
    fax: '',
    caring_employee: '',
    last_update: lastModifications ? lastModifications.lastModified : '',
    address_category4: '',
