...
module.exports.getExpression = msg => {
  ...
  let recordUid = (jsonata(`$filter(${JSON.stringify(msg.body.oihApplicationRecords)}, function($v) { $v.applicationUid = 2})`).evaluate()).recordUid;
  ...
  const expression = {
    ...
    rowid: ((recordUid === '' || recordUid === undefined)) ? '' : recordUid,
    oldstatus_remarks: '',
