...
module.exports.getExpression = msg => {
  ...
  let test = (jsonata(`$filter(${JSON.stringify(msg.body.oihApplicationRecords)}, function($v) { $v.applicationUid = 2})`).evaluate()).recordUid;
  ...
  const expression = {
    ...
    rowid: (((jsonata(`$filter(${JSON.stringify(msg.body.oihApplicationRecords)}, function($v) { $v.applicationUid = 2})`).evaluate()).recordUid === '') || ((jsonata(`$filter(${JSON.stringify(msg.body.oihApplicationRecords)}, function($v) { $v.applicationUid = 2})`).evaluate()).recordUid === undefined)) ? '' : test,
    oldstatus_remarks: '',
