...
module.exports.getExpression = msg => {
  ...
  // let appRecords = `${JSON.stringify(msg.body.oihApplicationRecords)}`;
  let recordUid;

  // msg.body.oihApplicationRecords.reduce(record => {
  //   let setIds = [];
  //   console.log('RECORD: ', record.recordUid);
  //   if (record.recordUid !== undefined) {
  //     console.log('FLAG 2');
  //     // recordUid = (jsonata(`$filter(${appRecords}, function($v) { $v.applicationUid = ''})`).evaluate()).recordUid;
  //     recordUid = record.recordUid;
  //     setIds.push(record.recordUid);
  //   }
  //   return setIds;
  // });
  msg.body.oihApplicationRecords.forEach(record => {
    let setIds = [];
    console.log('RECORD: ', record.recordUid);
    if (record.recordUid !== undefined) {
      console.log('FLAG 2');
  ...
  console.log(`ROWID: ${recordUid}`);

  // let recordUid = (jsonata(`$filter(${oihApplicationRecords}, function($v) { $v.applicationUid = ''})`).evaluate()).recordUid ?
  //    (jsonata(`$filter(${oihApplicationRecords}, function($v) { $v.applicationUid = 2})`).evaluate()).recordUid : '';
  ...
  const expression = {
    ...
    // rowid: (((jsonata(`$filter(${JSON.stringify(msg.body.oihApplicationRecords)}, function($v) { $v.applicationUid = 2})`).evaluate()).recordUid === '') || ((jsonata(`$filter(${JSON.stringify(msg.body.oihApplicationRecords)}, function($v) { $v.applicationUid = 2})`).evaluate()).recordUid === undefined)) ? '' : recordUid,
    rowid: recordUid ? recordUid : '',
    oldstatus_remarks: '',
