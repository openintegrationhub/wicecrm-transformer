...
module.exports.getExpression = msg => {
  ...
  console.log('STREET: '. msg.body.private_street);
  const expression = {
    oihUid: '',
    oihCreated: 746754,
    oihLastModified: '',
    ...
    // anniversary: '',
    addresses: [{
      street: (msg.body.private_street === null) ? '' : jsonata(`$trim($substringBefore("${msg.body.private_street}", $split("${msg.body.private_street}", ' ')[-1]))`).evaluate(),
      // streetNumber: (msg.body.private_street === null) ? '' : jsonata(`$number($split("${msg.body.private_street}", " ")[-1])`).evaluate(),
      // unit: '',
