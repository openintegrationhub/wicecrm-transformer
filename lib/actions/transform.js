...
function processAction(msg,cfg) {
  ...
  let test = cfg.body;
  console.log(`BODY: ${JSON.stringify(test, undefined, 2)}`);

  handlePassthrough(msg);
  console.log('Evaluating expression="%s" on body=%j', stringifiedExpression, expression);
  const result = compiledExpression.evaluate(expression);
